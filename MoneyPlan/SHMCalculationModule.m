...
{
    NSArray *TransactionsMadeByPerson = [self getTransactionsOfPerson:Person];
...
- (NSNumber *) debtOfPerson:(NSString *)debtor toPerson:(NSString *)owner
{
    ...
    double result = [DebtorToOwner doubleValue] - [OwnerToDebtor doubleValue];
    //return ([DebtorToOwner doubleValue] - [OwnerToDebtor doubleValue]) > 0? DebtorToOwner: [[NSNumber alloc] initWithDouble:0.0]; //правильно эту строку оставить
    
    NSNumber *debt = [[NSNumber alloc] initWithDouble:result];
...
{
    return [self.namesOfParticipantsWithId allKeys];
}
@end
